...
const HeroSection = () => {
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      {/* Animated Background Orbs — desktop only to save GPU on mobile */}
      <div className="hidden md:block absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary/10 rounded-full blur-3xl animate-float-slow" />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-primary/5 rounded-full blur-2xl animate-float-medium" style={{ animationDelay: "2s" }} />
      </div>

      {/* Gradient Overlay — desktop only */}
      <div className="hidden md:block absolute inset-0 bg-gradient-to-t from-background/50 via-transparent to-transparent animate-gradient-shift"
