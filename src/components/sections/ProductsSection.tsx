...
// Individual Card Component to handle its own scroll-linked animation
const StackedCard = ({ product, index, totalCards, scrollYProgress }: any) => {
  // Start animating this card when previous cards have finished.
  // We divide the total scroll progress into segments for each card.
  const step = 1 / totalCards;
  const startAnim = index * step;
  const endAnim = startAnim + step;

  // Transform values based on scroll progress:
  // - slide up from below (y: 100% -> 0%)
  // - scale down slightly as it goes to the "back" of the stack when subsequent cards arrive
  // - fade opacity slightly as it goes back
  const y = useTransform(scrollYProgress, [startAnim, endAnim], ["100%", "0%"]);

  // Cards that are already "placed" shrink slightly as new ones come over them
  const scale = useTransform(scrollYProgress, [endAnim, 1], [1, 0.9 + (index * 0.015)]);
  const opacity = useTransform(scrollYProgress, [endAnim, 1], [1, 0.5]);

  // The first card is always visible initially, no need to slide it up.
  ...
  return (
    <motion.div
      ...
      className={`absolute inset-0 w-full group flex flex-col h-full overflow-hidden rounded-2xl bg-slate-900 border border-slate-800 shadow-[0_[-10px]_20px_rgba(0,0,0,0.5)]`}
    >
      {/* Image Background */}
    ...
    <section id="products" className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        */}
        ...
        {/* Mobile View (Scroll-Jacking Sticky Stack) */}
        {/* We give the container a massive height to allow for scroll distance mimicking swiping through cards */}
        <div ref={containerRef} className="md:hidden relative h-[300vh]">
          {/* Sticky viewport that locks the cards in place while scrolling through the 300vh */}
          <div className="sticky top-24 pt-4 flex items-center justify-center h-[70vh] w-full overflow-hidden perspective-1000">
